...
async function commitImport(importId, groupId, userId) {
  ...
  // --- Collect all data first (pure JS, zero DB calls in this loop) ---
  const expensesToCreate = [];       // expense data rows
  const participantsByExpense = [];  // parallel array: splits for each expense
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (convertToSettlement.has(row.rowNumber)) {
        ...
        if (toUserId) {
          ...
          settlementsToCreate.push({
            groupId, fromUserId: payerId, toUserId,
            amount: Math.abs(amount), currency, exchangeRate,
            baseAmount: Math.abs(amount) * exchangeRate,
            notes: corrected.notes || corrected.description,
            importId, settledAt: expenseDate
          });
        }
        continue;
      }
      ...
      const participants = participantNames
        .map(name => { const uid = nameToUserId[name.toLowerCase()]; return uid ? { userId: uid, name } : null; })
        .filter(Boolean);
      ...
      if (splitType !== 'equal' && corrected.split_details) {
        ...
        if (splitType === 'unequal') {
          const amounts = {};
          for (const [name, val] of Object.entries(parsedDetails)) { const uid = nameToUserId[name.toLowerCase()]; if (uid) amounts[uid] = val; }
          splitDetails = { amounts };
        } else if (splitType === 'percentage') {
          const percentages = {};
          for (const [name, val] of Object.entries(parsedDetails)) { const uid = nameToUserId[name.toLowerCase()]; if (uid) percentages[uid] = val; }
          splitDetails = { percentages };
        } else if (splitType === 'share') {
          const shares = {};
          for (const [name, val] of Object.entries(parsedDetails)) { const uid = nameToUserId[name.toLowerCase()]; if (uid) shares[uid] = val; }
          splitDetails = { shares };
        }
      }
      ...
      expensesToCreate.push({
        groupId, paidById: payerId,
        amount: Math.abs(amount), currency, exchangeRate, baseAmount,
        description: corrected.description || 'Imported expense',
        expenseDate, splitType,
        notes: corrected.notes || null,
        importId
      });
      participantsByExpense.push(splits);
  ...
  // --- Batch insert everything: only 3 DB calls inside the transaction ---
  const { v4: uuidv4 } = require('uuid');

  await prisma.$transaction(async (tx) => {
    // 1. Settlements in one shot
    if (settlementsToCreate.length > 0) {
      await tx.settlement.createMany({ data: settlementsToCreate });
    }

    // 2. Expenses with pre-assigned IDs, then participants in one shot
    if (expensesToCreate.length > 0) {
      const expensesWithIds = expensesToCreate.map(e => ({ id: uuidv4(), ...e }));
      await tx.expense.createMany({ data: expensesWithIds });

      const allParticipants = expensesWithIds.flatMap((expense, idx) =>
        participantsByExpense[idx].map(s => ({
      ...
      if (allParticipants.length > 0) {
        await tx.expenseParticipant.createMany({ data: allParticipants });
      }
    }
  ...
  await prisma.import.update({
    ...
    data: {
      status: 'committed',
      processedRows: expensesToCreate.length + settlementsToCreate.length
    }
  });

  return {
    expensesCreated: expensesToCreate.length,
    settlementsCreated: settlementsToCreate.length,
    errors,
    total: rows.length,
